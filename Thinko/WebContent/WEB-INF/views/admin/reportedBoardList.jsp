...
<body>
	...
	<div class="container-fluid mt-5">
		<div class="row">
			...
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-12">
						<h2 class="title-1 m-b-25">신고된 게시글</h2>
							...
							<table class="table table-borderless table-striped purple-table">
								...
								<tbody>
									<c:choose>
										...
										<c:otherwise>
											<%-- 조회된 게시글 목록이 있을 때 --%>
											<c:forEach var="board" items="${rbList}">
												<tr class="text-center">
													<td>${board.boardNo}</td>
						...
						<c:choose>
							...
							<c:when test="${!empty param.sk && !empty param.sv }">
								<c:url var="pageUrl" value="/adminSearch/reportedBoardSearch.do" />
							...
							<c:otherwise>
								<c:url var="pageUrl" value="/admin/reportedBoardList.do" />
							</c:otherwise>
						</c:choose>
	...
	<div class="my-5">
		<form action="${contextPath}/adminSearch/reportedBoardSearch.do" method="GET"
	...
	<script>
		$(".table td").on("click", function() {
			...
			var url = "${contextPath}/board1/boardView.do?cp=${pInfo.currentPage}&no=" + boardNo + "${searchStr}&adrb=1&type=" + type;
